 ...
 * expenseslib.array_mapper_path subkey within the configuration that points
...
class ArrayMapperFactory
{
    public function __invoke($services)
    {
        if (! $services->has('config')) {
            throw new DomainException('Cannot create ExpensesLib\ArrayMapper; missing config dependency');
        }

        $config = $services->get('config');
        if (! isset($config['expenseslib']['array_mapper_path'])) {
            throw new DomainException(sprintf(
                'Cannot create %s; missing expenseslib.array_mapper_path configuration',
                ArrayMapper::class
            ));
        }

        $path = $config['expenseslib']['array_mapper_path'];
        if (! file_exists($path)) {
            throw new DomainException(sprintf(
